...
const Normalize = ({name,url,onExit}) => {
  ...
  const handleAlphaChange = async (event) => {
    ...
    setRemoteUrl(api.transform_normalize(name,event.target.value,beta));
  };
  const handleBetaChange = async (event) => {
    setBeta(event.target.value);
    setRemoteUrl(api.transform_normalize(name,alpha,event.target.value));
  };
