...
const Stylization = ({name,url,onExit}) => {
  ...
  const handleSigmaSChange = async (event) => {
    ...
    setRemoteUrl(api.transform_style(name,event.target.value,sigmaR));
  };
  const handleSigmaRChange = async (event) => {
    setSigmaR(event.target.value);
    setRemoteUrl(api.transform_style(name,sigmaS,event.target.value));
  };
