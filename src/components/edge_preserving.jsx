...
const EdgePreserving = ({name,url,onExit}) => {
  ...
  const handleSigmaSChange = async (event) => {
    ...
    setRemoteUrl(api.transform_edge_preserving(name,event.target.value,sigmaR));
  };
  const handleSigmaRChange = async (event) => {
    setSigmaR(event.target.value);
    setRemoteUrl(api.transform_edge_preserving(name,sigmaS,event.target.value));
  };
