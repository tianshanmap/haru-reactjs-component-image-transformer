//#region node_modules/haru-service-api/src/index.js
var API_BASE_URL_9080 = window.APP_CONFIG.API_URL_9080;
var API_BASE_URL_9081 = window.APP_CONFIG.API_URL_9081;
var API_BASE_URL_9082 = window.APP_CONFIG.API_URL_9082;
var CHUNK_SIZE = 5 * 1024 * 1024;
var callRemote = async (remote_url) => {
	try {
		const data = await (await fetch(remote_url)).json();
...
var api = {
	...
	deleteFile: (name, target = "") => {
		if (target == "") return callRemote(API_BASE_URL_9080 + "/delete?name=" + name);
		else return callRemote(API_BASE_URL_9080 + "/delete?name=" + name + "&parent=" + target);
	},
	...
	generateVideo: (request) => {
		const remote_url = API_BASE_URL_9082 + "/filesystem/video/generate/v1";
	...
	getNotes: async (name) => {
		...
		try {
			return (await (await fetch(remote_url)).json()).content;
		} catch (error) {
			console.error("Error fetching data:", error);
			return null;
		}
	...
	download_chunk: async (filename, onProgress) => {
		...
		if (download_filename == null) download_filename = filename;
		...
		createDownload(new Blob(chunks, { type: "application/octet-stream" }), download_filename);
	},
